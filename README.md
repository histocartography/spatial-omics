# spatial-omics
[![Build Status](https://travis.ibm.com/art-zurich/spatial-omics.svg?token=xpL9AKxpNBFpJzLTfSTv&branch=master)](https://travis.ibm.com/art-zurich/spatial-omics)
...
The spatialOmics contains unit tests in `tests/` directories in each subfolder.
This keeps the tests in close proximity to what they test. See the `setup.py` to include/exclude them with installation.

The test file spatialOmics/tests/test_module.py contains examples using the recommended `pytest`.
...
```sh 
python -m unittest discover -s spatialOmics -p "test_*" -v
```
...
```sh
black spatialOmics --check --diff --color
```
to apply changes:
```sh
black spatialOmics
...
- the [API page](https://pages.github.ibm.com/art-zurich/spatial-omics/api/spatialOmics.module.html) for [spatialOmics/module.py](spatialOmics/module.py).
- some [example](https://pages.github.ibm.com/art-zurich/spatial-omics/source/wealth_dynamics_md.html) additional documentation. Note that it can refer into the API.
...
Have a look a the example [slack channel for the spatialOmics bots](https://ibm-research.slack.com/archives/C02167RH2LW)


### Docker support

The spatialOmics contains a `Dockerfile` that builds an image containing the python package.
...
- `DOCKER_REGISTRY`: url of Artifactory registry. E.g.: `spatialOmics-docker-local.artifactory.swg-devops.com`.
...
```
DOCKER_BUILDKIT=1 docker build --ssh default -t spatialOmics:test .
```
